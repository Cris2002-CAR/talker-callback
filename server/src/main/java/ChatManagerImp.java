...
public class ChatManagerImp implements Demo.ChatManager {
    ...
    public void comand(String msg, CallbackPrx callbackPrx) {
        ...
        if (msg.contains("BC")) {
            ...
            try {
                ...
                if (number > 0) {
                    String fibResult = printFibonacciSequence(username, hostnamee, number);
                    String factorResult = printFactors(username, hostnamee, number);
                    String result = fibResult + "\n" + factorResult;
                    callbackPrx.printMsg(result);
                    successfulResponses++;
                    showStatistics();
                } else {
    ...
    // Calculate and Fibonacci of a number
    private static String printFibonacciSequence(String username, String hostname, int number) {
        StringBuilder result = new StringBuilder("Fibonacci sequence for " + number + " on " + username + "@" + hostname + ": ");
        int a = 0, b = 1, c;
        if (number >= 1) result.append("0 ");
        if (number >= 2) result.append("1 ");
